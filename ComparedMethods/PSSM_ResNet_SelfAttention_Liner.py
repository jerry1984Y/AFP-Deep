...
from utility import save_prob_label, create_list_train_test920_balance
...
class DCTModule(nn.Module):
    def __init__(self):
        ...
        self.res1=ResidualModule(64)

        self.cnn2 = nn.Conv1d(64, 256, 3)
        self.pool2 = nn.AvgPool1d(3)
        self.res2 = ResidualModule(256)

        self.cnn3 = nn.Conv1d(256, 128, 3)
        self.pool3 = nn.AvgPool1d(3)
        self.res3 = ResidualModule(128)

        self.atta=Self_Attention(128,128,128)
    ...
    def forward(self, pssm_feature,data_length):
        ...
        tsum = torch.sum(pssm, dim=1, keepdim=False)  # batchsize,L
        leg = torch.unsqueeze(ddl, dim=1)
        output = tsum / leg
...
def train():
    ...
    train_loader = DataLoader(dataset=train_set, batch_size=32, shuffle=True, num_workers=16, pin_memory=True,
...
def test():
    ...
    save_prob_label(arr_probs, arr_labels, '../Result/PSSM-Res-SA_920.csv')
    print('PSSM-Res-SA_920_' + b + '.csv', '<---------save to csv finish')
...
if __name__ == "__main__":
    ...
    lst_path_train_all, lst_label_train_all, test_path_all, test_label_all = create_list_train_test920_balance()
    train()
    test()
    print('completed')
