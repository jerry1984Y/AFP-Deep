...
class LstmModel(nn.Module):
    def __init__(self):
        ...
        self.emb = nn.Embedding(22, 20,padding_idx=21)
    ...
    def forward(self, seq_feature, data_length):
        #print('pssm_feature shape',pssm_feature.shape)
        pssm=self.emb(seq_feature)
...
def coll_paddding(batch_traindata):
    ...
    feature0 = torch.nn.utils.rnn.pad_sequence(feature0, batch_first=True, padding_value=21)
...
class BioinformaticsDataset(Dataset):
    ...
    def __init__(self, X, Y):
        ...
        model = np.array(['G', 'A', 'V', 'L', 'I', 'P', 'F', 'Y', 'W',
                          'S', 'T', 'C', 'M', 'N', 'Q', 'D', 'E', 'K', 'R', 'H', 'X','Z'])
        self.label_encoder.fit(model)
...
def test():
    ...
    print('auc', auc)
    print('<----------------')
    save_prob_label(arr_probs, arr_labels, 'Word Embedding.csv')
