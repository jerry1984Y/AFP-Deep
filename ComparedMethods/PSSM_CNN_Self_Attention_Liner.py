...
class DCTModule(nn.Module):
    ...
    def forward(self, pssm_feature,data_length):
        ...
        # pssm, output_lens = torch.nn.utils.rnn.pad_packed_sequence(pssm, batch_first=True)
        pssm=self.atta(pssm)
...
def test():
    ...
    save_prob_label(arr_probs, arr_labels, '../Result/PSSM-CNN-SA_920.csv')
    print('PSSM-CNN-SA_920_' + b + '.csv', '<---------save to csv finish')
